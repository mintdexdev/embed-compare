...
(function(L,U,N){let p=(a,ar)=>a.q.push(ar),d=L.document;L.Lunacal=L.Lunacal||function(){let lun=L.Lunacal,ar=arguments;if(!lun.loaded){lun.ns = {};lun.q=lun.q||[];const s=d.createElement("script");s.async=true;s.defer=true;s.src=U;d.head.appendChild(s);lun.loaded=!0}if(ar[0]===N){const api=function(){p(api, arguments)};const ns=ar[1];api.q=api.q||[];if(typeof ns==="string"){lun.ns[ns] = lun.ns[ns] || api;p(lun.ns[ns],ar);p(lun,["initNamespace",ns])}else p(lun,ar);return}p(lun,ar)};if(!L.Cal)L.Cal=L.Lunacal})(window,"https://app.lunacal.ai/embed/embed.js","init");Lunacal("init","multiple-durationssss111",{origin:"https://app.lunacal.ai"});
      ...
      <div className="flex flex-1 w-full max-w-3xl flex-col items-center justify-between py-32 px-16 bg-white dark:bg-black sm:items-start">
        ...
        <div className="flex flex-col items-center gap-6 text-center sm:items-start sm:text-left">
          <h1 className="max-w-xs text-3xl font-semibold leading-10 tracking-tight text-black dark:text-zinc-50">
            Lunacal embed loaded with async + defer.
          </h1>
          <p className="max-w-md text-lg leading-8 text-zinc-600 dark:text-zinc-400">
            The bootstrap snippet creates the embed.js script tag with{" "}
            <code className="font-mono">async</code> and{" "}
            <code className="font-mono">defer</code> set, so it does not block
            HTML parsing.
          </p>
        </div>
      </div>
